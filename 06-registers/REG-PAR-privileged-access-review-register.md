...
To provide the auditable evidence record that GIBB conducts the quarterly privileged access reviews required by [POL-06 Section 4.4.5](../02-policies/POL-06-access-control-policy.md). This is the register an internal or external auditor will request to verify the control is operating, not theoretical.
...
|---|---|---|---|---|---|---|
| Active Directory — Domain Admins | Head of IT Ops | 12 | 12 | 11 | 0 | 1 (departed staff retained in error from prior quarter — root cause: SOP-AC-01 step L3 latency on the day; fixed) |
...
- The single revocation in Active Directory Domain Admins traced back to a leaver event with delayed disablement. Root cause: SOP-AC-01 step L3 (automation) succeeded but a manual back-out by a separate workflow re-enabled the account. Process fix applied; verification scheduled into PAR-2026-Q2.
- Treasury / payments revocation related to a role change where the prior privileged role was not removed at the Mover step. Strengthened the [SOP-AC-01 Section 5.2](../04-procedures/SOP-AC-01-joiner-mover-leaver-sop.md) explicit removal language in the v1.1 minor update.
...
- **Source procedure:** [SOP-AC-01 Joiner / Mover / Leaver SOP](../04-procedures/SOP-AC-01-joiner-mover-leaver-sop.md)
- **SoA:** [REG-SOA](REG-SOA-statement-of-applicability.md) — control 5.18
