...
To define the operational steps the Security Operations Centre takes from the moment a security event is detected or reported through to: (a) classification per [STD-IR-01](../03-standards/STD-IR-01-incident-classification-and-severity-standard.md), (b) initial containment decision, (c) handover to incident response if severity warrants, or (d) closure if benign.

This SOP is the **first 60 minutes** of incident response. The [Incident Response Plan](../05-plans/PLN-01-incident-response-plan.md) covers what happens after triage handoff.
...
| Disable single user account | Confirmed credential compromise; align with [POL-06 Section 4.8](../02-policies/POL-06-access-control-policy.md) |
...
| T8 | SOC Shift Lead (or IC, once assigned) | Create entry in [Incident Register](../06-registers/REG-INC-incident-register.md) with assigned `INC-YYYY-NNN` identifier. | Incident Register | Incident recorded. | Within 30 min of severity confirmation. |
...
| C2 | SOC Tier 1/2 | Close SOAR ticket with closure category from STD-IR-01 Section 3.6, brief narrative, and lessons-learned tag if applicable. | SOAR | Ticket closed. | Same step. |
...
| # | Actor | Action | Tool | Output | SLA |
|---|---|---|---|---|---|
| C4 | SOC Tier 2 (designated IC for SEV-3) | Confirm containment, recovery, root cause; assess against materiality criteria (STD-IR-01 Section 3.2); if material, escalate to SEV-2 retrospectively and notify CISO. | SOAR + Incident Register | Closure narrative; materiality assessment. | Per MTTC target. |
...
| Incident Register entry | [REG-INC](../06-registers/REG-INC-incident-register.md) | Permanent (lifetime of bank) |
| Bridge recording (for SEV-1/SEV-2 once IRP activated) | Conferencing platform | 7 years |

## 8. Related documents

- **Parent standard:** [STD-IR-01 Incident Classification & Severity Standard](../03-standards/STD-IR-01-incident-classification-and-severity-standard.md)
